...
public class CalcDhontPais {
	...
	public List<Partido> calcular(){
		...
		for(Circunscripcion circ : circs) {
			List<ResultadosVotos>  elecs = ResultadosVotosDAOImplementation.getInstance().readResultado(anno, circ);
			int esc=AsignacionDAOImplementation.getInstance().readAsignacion(anno, circ);
			escPAIS = escPAIS + esc;
			for(ResultadosVotos elec : elecs) {
		...
		for(int i =0;i<escPAIS;i++) {
			...
			votos.set(maximoParcial, votosAux.get(maximoParcial)/(escannos.get(maximoParcial)+1));
